    ...
    for n=1:nparticles
        ...
        for beam=1:2         
            ...
            if beam==1  % rightmost beam
                j = 1;
                beam_angle = phi_min_laser;
            elseif beam==2  % leftmost beam
                j = 640;
                beam_angle = phi_max_laser;
            end
            ...
            if ~isnan(y_laser(i,j))
                ...
                y_measurement = y_laser(i,j);
                y_expected    = exp_meas(row_pos, col_pos, theta_particle(n)...
                    , beam_angle, ogp, ogres, threshold, y_laser_max);
            
                w_particle(n) = w_gain*w_particle(n)*normpdf(y_measurement,...
                    y_expected, sqrt(laser_var));
            
            end            % ------end of your particle filter weight calculation-------
        end
