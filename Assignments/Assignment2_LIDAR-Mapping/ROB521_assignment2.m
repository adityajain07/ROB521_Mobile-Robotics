...
for i=2:numodom
    theta_new = omega_odom(i)*(t_odom(i) - t_odom(i-1)) + theta_odom(i-1);
    ...
    x_new     = v_odom(i)*cos(theta_new)*(t_odom(i) - t_odom(i-1)) + x_odom(i-1);
    y_new     = v_odom(i)*sin(theta_new)*(t_odom(i) - t_odom(i-1)) + y_odom(i-1);
    
    x_odom(i) = x_new;
    y_odom(i) = y_new;
    theta_odom(i) = theta_new;
end
...
for n=1:2
    ...
    x_map = [];
    y_map = [];
    laser_angles = linspace(phi_min_laser, phi_max_laser, size(y_laser, 2))';
        ...
        for j=1:size(y_laser, 2)   
            if omega_interp(i)<0.1
                if ~isnan(y_laser(i, j))               
                    ...
                    x_botframe = (y_laser(i, j)-0.1)*cos(theta_interp(i) + laser_angles(j));
                    y_botframe = (y_laser(i, j)-0.1)*sin(theta_interp(i) + laser_angles(j));
            
                    x_inerframe = x_interp(i) + x_botframe;
                    y_inerframe = y_interp(i) + y_botframe;
            
                    x_map = [x_map x_inerframe];
                    y_map = [y_map y_inerframe];
    ...
    if n==1
        scatter(x_map,y_map, 5,'b', 'filled');
        print -dpng self_ass1_q3_gt.png
        hold on
    else
        axis equal;
        scatter(x_map,y_map, 5,'r', 'filled');
    end
end

legend('groundtruth pose','noisyodometry pose')
xlabel('x [m]');
ylabel('y [m]');
title('map');
print -dpng self_ass1_q3_both.png
