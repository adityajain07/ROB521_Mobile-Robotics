...
class PygameWindow:
    def __init__(self,
        ...
        map_img = pygame.image.load('../maps/' + map_name)
    ...
    def add_point(self, map_frame_point, radius=4, width=4, color=COLORS['r']):
    ...
    def add_line(self, p1, p2, width=3, color=COLORS['b']):
        p1[1] = -p1[1]  # for top left origin
        p2[1] = -p2[1]  # for top left origin
        p1 = self.point_to_vec(np.array(p1) / self.meters_per_pixel + self.origin_pixels)
        p2 = self.point_to_vec(np.array(p2) / self.meters_per_pixel + self.origin_pixels)
        pygame.draw.line(self.screen, color, p1, p2, width)
        pygame.display.update()

    def add_line_start_to_goal(self, p1, p2, width=3, color=COLORS['r']):
        p1[1] = -p1[1]  # for top left origin
        p2[1] = -p2[1]  # for top left origin
        p1 = self.point_to_vec(np.array(p1) / self.meters_per_pixel + self.origin_pixels)
        p2 = self.point_to_vec(np.array(p2) / self.meters_per_pixel + self.origin_pixels)
    ...
    def remove_line(self, p1, p2, width=3, color=COLORS['w']):
        p1[1] = -p1[1]  # for top left origin
        p2[1] = -p2[1]  # for top left origin
        p1 = self.point_to_vec(np.array(p1) / self.meters_per_pixel + self.origin_pixels)
        p2 = self.point_to_vec(np.array(p2) / self.meters_per_pixel + self.origin_pixels)
